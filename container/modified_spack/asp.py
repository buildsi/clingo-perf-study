...
class PyclingoDriver(object):
    ...
    def solve(
        ...
        self.control.configuration.configuration = conf # 'tweety'
...
def solve(specs, dump=(), models=0, timers=False, stats=False, tests=False,
          reuse=False, rtimer=False, coref=False, conf='tweety'):
    ...
    """
    driver = PyclingoDriver(cores=coref)
    if "asp" in dump:
    ...
    (res, timer, solve_stat) = driver.solve(
        setup, specs, dump, models, timers, stats, tests, reuse, conf
    )
    if rtimer:
        return (res, timer, setup.len_pkgs, solve_stat)
    return res
