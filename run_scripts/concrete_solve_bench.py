...
if __name__ == "__main__":
    ...
    timer = specs[0].concretize()
    # print("{0} -- {1} -- {2}".format('hdf5', str(timer.phases["concretizer"]), specs[0].concrete))
    # specs[0].to_json('hdf5.json')
    # with open('hdf5.json', 'w') as fd:
    #    specs[0].to_json(fd)
    # exit()
    ...
    if single_pkg and mrank == 0:
        ...
        timer = specs[0].concretize()
        timer.stop()
        print('{0}, {1}, {2}'.format(single_pkg, int(specs[0].concrete), str(timer.phases["concretizer"])))
        # for cf in configs:
                ...
                for cf in configs:
                    for i in range(rp):
                        try:
                            ...
                            timer = specs[0].concretize()
                            timer.stop()
                            print('{0}, {1}, {2}, {3}'.format(pkg, i, int(specs[0].concrete), str(timer.phases["concretizer"])))
                            # sol_res, timer, len_pkgs, solve_stat = asp.solve(specs, timers=False, reuse=reusef, rtimer=True, coref=coref, conf=cf)
                            # timer.stop()
                            ...
                            with open('jsons/{0}.json'.format(pkg), 'w') as fd:
                                specs[0].to_json(fd)
                        except:
                            print('{0}, {1}, {2}, {3}'.format(pkg, i, 0, 0))
                            sys.stdout.flush()
